...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\0cifar\\cifar10_automobile_train_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    readText = visual.TextStim(win=win, name='readText',
        text='Please read the word "Automobile".',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "Part3_first" ---
    ...
    # --- Prepare to start Routine "Part3_first" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Part3_first.started', globalClock.getTime(format='float'))
    # keep track of which components have finished
    Part3_firstComponents = [text_2]
    for thisComponent in Part3_firstComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "Part3_first" ---
    for thisComponent in Part3_firstComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Part3_first.stopped', globalClock.getTime(format='float'))
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
    else:
    ...
    trials_4 = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('cifar10png/automobile_train.xlsx', selection='0:1000'),
    ...
    while continueRoutine and routineTimer.getTime() < 30.0:
        ...
        if quicklookImage.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > quicklookImage.tStartRefresh + 30-frameTolerance:
                # keep track of stop time/frame for later
    ...
    trials_5 = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('cifar10png/automobile_train.xlsx', selection='1001:2000'),
    ...
    trials_6 = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('cifar10png/automobile_train.xlsx', selection='2001:3000'),
    ...
    trials_7 = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('cifar10png/automobile_train.xlsx', selection='3001:4000'),
    ...
    trials_8 = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('cifar10png/automobile_train.xlsx', selection='4001:5000'),
